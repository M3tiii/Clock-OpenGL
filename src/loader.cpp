...
Model getOBJinfo(string fp)
{
    ...
    if(!inOBJ.good())
    {
        cout << "ERROR OPENING OBJ FILE: " << fp <<endl;
        exit(1);
    }
...
void writeCvertices(string fp, string name, Model model)
{
    ...
    // Header
//    outC << "#include " << "\"" << name << ".h" << "\"" << endl; //ZMIANA BEZ .H !!! //MATI
//    outC << endl;

    // Vertices
    outC << "int " << name << "Vertices = " << model.vertices << ";" << endl;
    outC << endl;
...
void writeCpositions(string fp, string name, Model model, int faces[][9], float positions[][3])
{
    ...
    // Positions
    outC << "float " << name << "Positions[" << model.vertices*3 << "] = " << endl;
    outC << "{" << endl;
...
void writeCtexels(string fp, string name, Model model, int faces[][9], float texels[][2])
{
    ...
    // Texels
    outC << "float " << name << "Texels[" << model.vertices*2 << "] = " << endl;
    outC << "{" << endl;
    for(int i=0; i<model.faces; i++)
    {
        int vtA = faces[i][1] - 1;
...
int createModel(string pathhh, string nameObjjj)
{
    // Files
    string nameOBJ = nameObjjj;
    string filepathOBJ = pathhh + "source/" + nameOBJ + ".obj";
    string filepathH = pathhh + "product/" + nameOBJ + ".h";
    string filepathC = pathhh + "product/" + nameOBJ + ".c";
